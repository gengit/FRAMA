...
perl sge.pl -db name -query file.fa -out myresult -jobs 2 [-format xml -flavor
...
my %opt = (
    ...
    jobs      => 10,
    path      => {
        "wu"           => "/gsc/biosw/src/wublast/",
        "ncbi"         => "/gsc/biosw/bin/blastall",
        "megablast"    => "/gsc/biosw/bin/megablast",
        "blat"         => "/gsc/biosw/bin/blat",
        "repeatmasker" => "/gsc/biosw/bin/RepeatMasker",
...
#unless (-e $opt{db} || $opt{flavor} eq "repeatmasker") {
...
if ($opt{engine} eq "parallel" || $opt{engine} eq "sge") {
    ...
    open my $out_seq, ">", $out_file or die "Can't open file for writing: $out_file\n";
    ...
    while (my $seq = $in->next_seq) {
        ...
        if ($seq_count <= $seq_per_file) {
            ...
            close $out_seq;
            open $out_seq, ">", $out_file or die "Can't open file for writing: $out_file\n";
    ...
    if ($opt{engine} eq "parallel") {
        ...
        my $file = File::Spec->catfile($opt{tmp}, $rand_string . "_commands.txt");
        ...
        my @options =
          ("#!" . $opt{path}->{bash}, "#\$ -t 1-$opt{jobs}", "#\$ -o $sge_out", "#\$ -e $sge_err");
        my @commands = ("i=\$(expr \$SGE_TASK_ID - 1)", $blast_command, "exit 0");
        ...
        if ($output_line =~ /Your job-array (\d+)\.(.+?) \((.+?)\) has been submitted/) {
            $jobid = $1;
        } else {
            removeFiles(1);
        }
        ...
        if ($opt{flavor} eq "repeatmasker") {
            @output_files =
              map { File::Spec->catfile($opt{tmp}, $rand_string . "_input.$_.masked") }
              0 .. ($opt{jobs} - 1);
        } else {
            @output_files = map { File::Spec->catfile($opt{tmp}, $rand_string . "_output.$_") }
              0 .. ($opt{jobs} - 1);
        }
    }
...
sub getCommand {
    ...
    my $command;
    if ($opt{flavor} eq 'wu') {
        $command = $opt{path}->{$opt{flavor}} . "$opt{type} $opt{db} $input -o=$output $formats{$opt{format}} $opt{parameter}";
    } elsif ($opt{flavor} eq 'megablast') {
        $command = $opt{path}->{$opt{flavor}} . " -d $opt{db} -i $input -o $output $formats{$opt{format}} $opt{parameter}";
    } elsif ($opt{flavor} eq 'blat') {
        $command = $opt{path}->{$opt{flavor}} . " $opt{db} $input $output $formats{$opt{format}} $opt{parameter} &> /dev/null";
    } elsif ($opt{flavor} eq 'ncbi') {
        $command = $opt{path}->{$opt{flavor}} . " -p $opt{type} -d $opt{db} -i $input -o $output $formats{$opt{format}} $opt{parameter}";
    } elsif ($opt{flavor} eq 'repeatmasker') {
        $command = "cd $opt{tmp} && " . $opt{path}->{$opt{flavor}} . " $input $opt{parameter}";
        $output  = $input . ".masked";
    }
...
sub combineResults {
    ...
    if ($opt{flavor} eq "repeatmasker") {
        ...
        $command = "cat " . join(" ", @files) . " > $opt{out};";
        my @blast_files = map { s/output/input/; $_ . ".blast" } @files;
        ...
        if ($opt{format} =~ /tabular/) {
            ...
            $command = "cat " . join(" ", @files) . " > $opt{out}";
        } elsif ($opt{format} =~ /xml/) {
            $command = "blast_combineXML.py $opt{out} " . join(" ", @files);
        }
    }
